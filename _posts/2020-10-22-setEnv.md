...
거의 10년이 넘게 유용하게 사용합니다.  

사람의 기억력은 미화하거나, 잊혀지지만, 잘 정리된 글은 필요할 때, 다시 봐도 변하지 않아 매우 뿌듣합니다.  

예전 tomcat 의 서버 실행의 경우,
별도의 tomcat 폴더의 bin 에 있는 catalina.sh 을 실행하여,

CATALINA_OPTS 같은 변수를 export 했습니다.  

jconsole 등을 사용하기 위해 xmxremote 등을 설정하는 독자적인 설정을 각 sh 파일에 export 해왔는데요.  

스프링부트로 단독으로 올리게 되어, 아예 start 와 stop shell 을 구성하게 되었습니다.  
...
```
scouter 로 트래픽 및 성능 및 모니터링을 해야하므로, 기본 설정값으로
